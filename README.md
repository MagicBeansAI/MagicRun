...
| **Process isolation** | **Optional, platform-dependent** | `execute_batch_in_jail` uses an explicitly prepared jail. macOS/Linux implementations have platform prerequisites; unsupported jail requests fail closed. Ordinary batch execution is not automatically jailed. The opt-in brokered-egress jail admits exactly one host-owned HTTP CONNECT broker and no other network ([details](docs/architecture.md#brokered-egress-process-jail)). |
